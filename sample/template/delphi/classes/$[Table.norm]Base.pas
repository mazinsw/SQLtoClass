...
$[field.else.if(enum)]
    ...
    /// </summary>
    /// <param name="Value">Texto que ser� convertido
    /// </param>
    /// <returns>T$[Table.norm]$[Field.norm] se conseguir;
    /// caso contr�rio uma Exception ser� lan�ada.
    /// </returns>
...
$[field.end]
    ...
    /// <summary>
    ///   Atribui todos os valores do par�metro Source para a inst�ncia pr�pria
    /// </summary>
    /// <param name="Source">Inst�ncia de onde os valores ser�o copiados
    /// </param>
...
begin
  ...
  raise Exception.CreateFmt('N�o foi poss�vel converter de "%s" para T$[Table.norm]$[Field.norm]', [Value]);
end;
$[field.end]
$[field.end]
...
begin
  if (Index < 1) or (Index > $[field.array.count]) then
    raise Exception.CreateFmt('�ndice %d inv�lido, m�nimo %d, m�ximo %d', [Index, 1, $[field.array.count]]);
