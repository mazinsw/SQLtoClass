...
$[field.if(repeated)]
$[field.else.if(array)]
    function Get$[Field.norm](Index: Integer): string;
    procedure Set$[Field.norm](Index: Integer; Value: string);
$[field.else.if(blob)]
...
$[field.end]
$[field.end]
    procedure Assign(Value: T$[Table.norm]Base);
$[field.each(all)]
$[field.if(repeated)]
...
procedure T$[Table.norm]Base.Assign(Value: T$[Table.norm]Base);
$[table.exists(array|enum)]
var
  I: Integer;
$[table.end]
...
$[field.else.if(array)]
  for I := 1 to $[field.array.count] do
    F$[Field.norm][I] := Value.F$[Field.norm][I];
$[field.else.if(blob)]
  Set$[Field.norm](Value.F$[Field.norm]);
$[field.else]
  F$[Field.norm] := Value.F$[Field.norm];
$[field.end]
$[field.end]
end;
$[field.each(all)]
...
function T$[Table.norm]Base.Get$[Field.norm](Index: Integer): string;
begin
...
procedure T$[Table.norm]Base.Set$[Field.norm](Index: Integer; Value: string);
begin
  if (Index < 1) or (Index > $[field.array.count]) then
    raise Exception.CreateFmt('Índice %d inválido, mínimo %d, máximo %d', [Index, 1, $[field.array.count]]);
